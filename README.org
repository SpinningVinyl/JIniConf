...
~IniConfReader~ decodes files as UTF-8. Its ~read()~ method returns ~Optional.empty()~ if an I/O error occurs while
reading the file, including when the file does not contain valid UTF-8.
...
Or use ~IniConfWriter~ to save a UTF-8 encoded =.INI= file:

#+begin_src java
IniConfWriter.write("sample.ini", iniConf);
#+end_src
